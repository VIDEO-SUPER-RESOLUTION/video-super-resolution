...
def show_lr(path):
    popup_lr = tk.Tk()
    popup_lr.wm_title("Low Resolution VIDEO")

    label = ttk.Label(popup_lr, justify=tk.LEFT, text="""Original Low Resolution VIDEO""", font=("Verdana", 14, "bold"))
    ...
    B1 = ttk.Button(popup_lr, text="SELECT FOLDER TO SAVE THIS VIDEO", command=lambda: save_file(img, path, scale=1))
    B1.pack(side="top")
...
def show_sr(scale=2):
    popup_sr = tk.Tk()
    if scale == 2:
        popup_sr.wm_title("Super Resolved VIDEO x2")

        label = ttk.Label(popup_sr, justify=tk.CENTER, text="""Super Resoved VIDEO x2""", font=("Verdana", 14, "bold"))
        label.pack(side="top", fill="x", pady=10, padx=30)
    elif scale == 4:
        popup_sr.wm_title("Super Resolved VIDEO x4")

        label = ttk.Label(popup_sr, justify=tk.LEFT, text="""Super Resoved VIDEO x4""", font=("Verdana", 14, "bold"))
    ...
    B1 = ttk.Button(popup_sr, text="SELECT FOLDER TO SAVE THIS VIDEO", command=lambda: save_file(img, path, scale=scale))
    B1.pack(side="top")
...
label = ttk.Label(root, justify=tk.CENTER,
                  text="Click the browse button below to select the video file", font=("Verdana", 11))
...
label = ttk.Label(root, justify=tk.CENTER, text="Path of the selected video file", font=("Verdana", 11))
...
button1 = ttk.Button(root, text="SHOW ORIGINAL VIDEO", command=lambda: show_lr(global_filename))
button1.pack()

label = ttk.Label(root, justify=tk.CENTER, text="")
label.pack(side="top", pady=2, padx=30)

button2 = ttk.Button(root, text="SUPER RESOLOVE X2", command=lambda:  ff.main(global_filename))
button2.pack()
